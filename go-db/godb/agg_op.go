...
func (a *Aggregator) Iterator(tid TransactionID) (func() (*Tuple, error), error) {
	...
	if childIter == nil {
		return nil, GoDBError{MalformedDataError, "Aggregator child Iterator unexpectedly nil."}
	}
	...
	if a.groupByFields == nil {
		...
		for _, as := range a.newAggState {
			copy := as.Copy()
			if copy == nil {
				return nil, GoDBError{MalformedDataError, "Copy of aggState unexpectedly returned nil."}
			}
