...
func TestReadNonExistantPage(t *testing.T) {
	_, t1, _, hf, _, tid := makeTestVars()
	hf.insertTuple(&t1, tid)
	_, err := hf.readPage(100)
	if err == nil {
		t.Errorf("HeapFile allowed reading non-existant page.")
	}
}
...
func TestDeleteFromWrongFile(t *testing.T) {
	td, tup, _, hf, bp, tid := makeTestVars()
	_, tupDup, _, _, _, _ := makeTestVars()

	os.Remove(TestingFile2)
	hf2, _ := NewHeapFile(TestingFile2, &td, bp)

	hf.insertTuple(&tup, tid)
	hf2.insertTuple(&tupDup, tid)

	// tup2 should be specific to hf2, so line below should give TupleNotFoundError
	err := hf.deleteTuple(&tupDup, tid)
	if err == nil {
		// TODO(tally): check error codes and that record was actually deleted
		t.Errorf("Allowed delete with wrong recordId.")
	}
}
