...
func (dop *DeleteOp) Iterator(tid TransactionID) (func() (*Tuple, error), error) {
	...
	if childIter == nil {
		return nil, GoDBError{MalformedDataError, "DeleteOp child Iterator unexpectedly nil."}
	}
	if !dop.child.Descriptor().equals(dop.file.Descriptor()) {
		return nil, GoDBError{TypeMismatchError, "Trying to delete tuples with wrong type from table."}
	}
	complete := false
