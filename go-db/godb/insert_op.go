...
func (iop *InsertOp) Iterator(tid TransactionID) (func() (*Tuple, error), error) {
	...
	if childIter == nil {
		return nil, GoDBError{MalformedDataError, "InsertOp child Iterator unexpectedly nil."}
	}
	if !iop.child.Descriptor().equals(iop.file.Descriptor()) {
		return nil, GoDBError{TypeMismatchError, "Trying to insert tuples with wrong type into table."}
	}
	complete := false
