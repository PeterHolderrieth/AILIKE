...
// 					if table != "" {
// 						return "", ailikeError{AmbiguousNameError, fmt.Sprintf("multiple possible table names for field %s in select expression", field)}
// 					}
...
// 				fieldName = newFieldName
// 			} else if newTabName != "" {
// 				return "", "", ailikeError{AmbiguousNameError, fmt.Sprintf("multiple possible table names for field %s in select expression", fieldName)}
// 			}
// 		}
...
// 	subqueries    []*LogicalPlan //Other subqueries to call
...
// 			if op != OpEq {
// 				return nil, nil, ailikeError{IllegalOperationError, "only equality joins are supported"}
// 			}
...
// 		}
// 	default:
// 		return nil, nil, ailikeError{ParseError, "where expression with non value or column on RHS (disjunctions and nested where expressions are not supported)"}
// 	}
// }

// // The following function parses a SQL "FROM" statement
// // If the table expression is
// // 1. A simple table (base table): simply return that table (no joins, no logical plans)
// // 2. A subquery: if it is a subquery, the simply add it to LogicalPlan
// // 3. A parent table expression (i.e. a tuple of table), then parse each individual table.
...
// 			}
// 		// For a simple table expression, we simply only return the table.
...
// 			return nil, nil, nil, ailikeError{ParseError, fmt.Sprintf("unsupported join type %s", joinTable.Join)}
// 		}
...
// 	}
// 	return nil, nil, nil, ailikeError{ParseError, "unknown query type in parseFrom"}
// }
...
// 		if isAgg(funName) {
// 			if len(expr.Exprs) != 1 {
// 				return nil, ailikeError{ParseError, fmt.Sprintf("expected one argument to aggregate %s in select list", sqlparser.String(expr.Name))}
// 			}
// 			star, ok := expr.Exprs[0].(*sqlparser.StarExpr)
// 			if ok {
// 				if funName != "count" {
// 					return nil, ailikeError{ParseError, "got * in non-count aggregate"}
// 				}
...
// 		return &field, nil
// 	default:
// 		return nil, ailikeError{ParseError, fmt.Sprintf("unsupported expression type %s in select list", reflect.TypeOf(expr))}
// 	}
...
// 	default:
// 		return nil, ailikeError{ParseError, fmt.Sprintf("unsupported expression type %s in select list", reflect.TypeOf(exprAlias))}
...
// // Function to parse a statement step-by-step
// // 1. Create base tables (s.From)
// // 2. Process WHERE statements (filters and joins)
// // 3. Process selection expressions (aggregrats and projects)
// // 4. Append groupBy's
// // 5. Append orderBy's
// // 6. Parse LIMIT statement
...
// 	if node == nil && tab != "" {
// 		return nil, ailikeError{ParseError, fmt.Sprintf("no table in catalog matching '%s'", tab)}
// 	}
// 	if node == nil {
...
// 						return nil, ailikeError{ParseError, fmt.Sprintf("field name '%s' is ambiguous", field)}
// 					}
// 				}
// 			}
// 		}
// 	}
// 	if node == nil {
// 		return nil, ailikeError{ParseError, fmt.Sprintf("no field in catalog matching '%s'", field)}
// 	}
// 	return node, nil
// }
...
// 		return best, nil
// 	}
// 	return FieldType{}, ailikeError{ParseError, fmt.Sprintf("no field in catalog matching '%s'", field)}
// }
...
// 		var field FieldType
// 		if inputDesc == nil {
// 			return nil, "", ailikeError{ParseError, "Tuple desc must be non-null for expression fields"}
// 		}
// 		if tableMap == nil {
// 			return nil, "", ailikeError{ParseError, "Table map must be non-null for expression fields"}
// 		}
...
// 				if err != nil {
// 					return nil, "", ailikeError{ParseError, fmt.Sprintf("cannot select field %s that is not child expression", s.field)}
// 				}
// 			} else {
...
// 	}
// 	return nil, "", ailikeError{ParseError, "unhandled expression type in select list"}
...
// // Function to create a physical plan from a logical plan
// // The logical plan is given by the LogicalPlan structure
// // 1. For each subsquery (or subplan), a physical plan is created (function is recursively called)
// // 2. For all basetables, heapfile are added as operators
// // 3. For all filters, they are applied to the above tables
// // 4. Joins are performed and table map is executed
// // 5. Aggregates are added
// // 6. Projections are added
// // 7. OrderBy are added
// // Finally, it returns an operator that calls everything in the correct order.
...
// 		} else {
// 			if curOp != node.op {
// 				return nil, ailikeError{ParseError, "not all tables are joined, cross products are not supported in ailike"}
// 			}
// 		}
// 	}

// 	//topOp is the top operator in the tree. This is the one that will be returned.
// 	topOp := curOp
...
// 					as = &CountAggState{}
// 				default:
// 					return nil, ailikeError{IllegalOperationError, fmt.Sprintf("unknown aggregate function %s", *s.funcOp)}
// 				}
...
// 		return nil, ailikeError{ParseError, "ailike doesn't support inserts of incomplete tuples"}
// 	}
...
// 		return nil, ailikeError{ParseError, "ailike does not supporting deleting from multiple tables"}
// 	}
...
// 		if t != nil {
// 			return UnknownQueryType, ailikeError{ParseError, fmt.Sprintf("table %s already exists", tabName)}
// 		}
...
// 				colType = StringType
// 			default:
// 				return UnknownQueryType, ailikeError{ParseError, fmt.Sprintf("unsupported column type %s", col.Type.Type)}
...
// 	default:
// 		return UnknownQueryType, ailikeError{ParseError, fmt.Sprintf("unsupported ddl statement %s", ddl.Action)}
// 	}
// }

// // Functions that parses statements:
// // I.e. it accepts a SQL query and returns an operator.
// // More specifically, it returns a query type, an operator, and an error
// //1. For insert, delete, and select statements, queryType == IteratorType
// //2. For transaction commands, it returns their corresponding query types (Begin, Commit, Abort)
// //3. For a DDL statement (CREATE, ALTER, DROP, RENAME or TRUNCATE) - i.e. changing the physical layout of tables -
// //a corresponding query type is returned that causes the calling function to update the catalogue. In addition,
// //the actual command is immediately executed here (not later upon calling the iterator) - so no operator is returned.
// //see processDDL.
// //Called functions:
// // SELECT:
// // - parseStatement (understood)
// // - makePhysicalPlan (understood)
// // Insert:
// // - parseInsert
// // DELETE:
// // - parseDelete
...
// 	case *sqlparser.DDL: //Drop, alter, etc. statements
...
// 	return UnknownQueryType, nil, ailikeError{ParseError, "invalid query"}
// }
