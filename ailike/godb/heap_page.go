...
In ailike all tuples are fixed length, which means that given a TupleDesc it is
...
unsafe.Sizeof() to determine the size in bytes of an object.  So, a ailike integer
...
disk, tuples should retain the same slot number. Because ailike will never evict a
...
func (h *heapPage) insertTuple(t *Tuple) (recordID, error) {
	if h.numOpenSlots == 0 {
		return nil, ailikeError{PageFullError, "No empty slots in heap page."}
	}
	...
	return nil, ailikeError{PageFullError, "No empty slots in heap page; page is malformed."}
}
...
func (h *heapPage) findTuple(rid recordID) (*Tuple, error) {
	...
	return nil, ailikeError{IllegalOperationError, "Trying to find a non-existant tuple."}
}
...
func (h *heapPage) deleteTuple(rid recordID) error {
	...
	return ailikeError{IllegalOperationError, "Trying to delete a non-existant tuple."}
}
