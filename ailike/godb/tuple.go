...
// DBType is the type of a tuple field, in ailike, e.g., IntType or StringType
type DBType int
...
// FieldType is the type of a field in a tuple, e.g., its name, table, and [ailike.DBType].
...
func findFieldInTd(f FieldType, desc *TupleDesc) (int, error) {
	...
	for i, fi := range desc.Fields {
		if fi.Fname == f.Fname && (fi.Ftype == f.Ftype || f.Ftype == UnknownType) {
			if f.TableQualifier == "" && best != -1 {
				return 0, ailikeError{AmbiguousNameError, fmt.Sprintf("select name %s is ambiguous", fi.Fname)}
			}
	...
	return -1, ailikeError{IncompatibleTypesError, fmt.Sprintf("field %s.%s not found", f.TableQualifier, f.Fname)}
}
...
func (desc *TupleDesc) getNumSlotsPerPage(pagesize int) (int32, error) {
	...
	if bytes_per_tuple == 0 {
		return -1, ailikeError{MalformedDataError,
	...
	for i, f := range t.Fields {
		...
		case StringField:
			if t.Desc.Fields[i].Ftype != StringType {
				return ailikeError{TypeMismatchError, "Tuple's fields do not match its descriptor."}
			}
...
//
// Note that in most cases Expr will be a [ailike.FieldExpr], which simply
...
func (t *Tuple) compareField(t2 *Tuple, field Expr) (orderByState, error) {
	...
	return OrderedEqual, ailikeError{IllegalOperationError, "Cannot compare expressions with unknown type."}
}
...
func (t *Tuple) project(fields []FieldType) (*Tuple, error) {
	...
	for i, ft := range fields {
		...
		if err != nil {
			if err.(ailikeError).code == IncompatibleTypesError {
