...
func NewProjectOp(selectFields []Expr, outputNames []string, distinct bool, child Operator) (Operator, error) {
	if len(selectFields) != len(outputNames) {
		...
		return nil, ailikeError{MalformedDataError, "selectFields and outputNames have different lengths."}
	}
	if child == nil {
		return nil, ailikeError{MalformedDataError, "NewProjectOp child pointer is nil."}
	}
...
func (p *Project) Iterator(tid TransactionID) (func() (*Tuple, error), error) {
	...
	if childIter == nil {
		return nil, ailikeError{MalformedDataError, "ProjectOp child Iterator unexpectedly nil."}
	}
