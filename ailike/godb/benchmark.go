...
func BenchmarkingInfra(queryName string, query string, config BenchMetaData) (time.Duration, error) {
	...
	if plan == nil {
		return time.Duration(0), ailikeError{ParseError, "Plan was nil"}
	}
	if qType != IteratorType {
		return time.Duration(0), ailikeError{ParseError, "Plan is not of iterator type"}
	}
	desc := plan.Descriptor()
	if desc == nil {
		return time.Duration(0), ailikeError{ParseError, "Descriptor was nil"}
	}
	tid := NewTID()
	...
	if config.save {
		...
		if err != nil {
			return end, ailikeError{OSError, err.Error()}
		}
...
func RunWikiArticleQuery(query string, c *Catalog, colNo int, bp *BufferPool) ([]int64, error) {
	...
	if plan == nil {
		return nil, ailikeError{ParseError, "Plan was nil"}
	}
	if qType != IteratorType {
		return nil, ailikeError{ParseError, "Plan is not of iterator type"}
	}
	desc := plan.Descriptor()
	if desc == nil {
		return nil, ailikeError{ParseError, "Descriptor was nil"}
	}
	tid := NewTID()
	iter, err := plan.Iterator(tid)
	if err != nil {
