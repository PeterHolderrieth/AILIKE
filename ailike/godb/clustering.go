...
func (c *Clustering) addRecordToClustering(rid recordID, emb *EmbeddingType) (int, float64, error) {
	...
	if c.NCentroids() < c.maxNClusters {
		...
		if _, ok := c.centroidEmbs[newCentroidId]; ok {
			return 0, 0.0, ailikeError{errString: "Expected centroid to not exist but it exists.",
...
func (c *Clustering) updateSingleCentroidVector(clusterID int, newEmb *EmbeddingType, newSumDist *float64) error {
	...
	if _, ok := c.clusterMemb[clusterID]; !ok {
		return ailikeError{errString: "Do not know this clusterID.", code: UnknownClusterError}
	}

	//If an embedding is given, then simply insert it
	if newEmb != nil {
		if newSumDist == nil {
			return ailikeError{errString: "If newEmb is given, also newSumDist must be given.", code: TypeMismatchError}
		}
		...
		if !c.storeEmbs {
			return ailikeError{errString: "Embeddings can only be automatically updated if c.storeEmbs is true (need to store embeddings).", code: TypeMismatchError}
		}

		if newSumDist != nil {
			return ailikeError{errString: "If a newSumDist is given, also newEmb must be given.", code: TypeMismatchError}
		}
	...
	for (nIteration < maxIterations) && (true) {
		...
		for clusterID, members := range clustering.clusterMemb {
			...
			if _, ok := CentroidMap[clusterID]; !ok {
				if nMembers != 0 {
					return nil, ailikeError{errString: "The centroid map does not have that cluster registered but the cluster has more than 0 members.",
						code: UnknownClusterError}
				}
